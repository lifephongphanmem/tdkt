...
class LapHoSoTdctController extends Controller
{
    public function boxungdt(Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        $modeltdct = new laphosotdct();
        $inputs['madktdct'] = getdate()[0];
        $modeltdct->create($inputs);
        $m_pl = dmphanloaict::all();
        $modelct = laphosotdct::where('kihieudhtd',$inputs['kihieudhtd'])
            ->get();
        $slcanhan = 0;
        $sltapthe = 0;
    ...
    public function delete(Request $request){
        ...
        if(isset($inputs['id'])){
            ...
            $model = laphosotdct::where('id',$inputs['id'])->first();
            $model->delete();
    ...
    public function chinhsuadt (Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        if(isset($inputs['id'])){
            $id = $inputs['id'];
            $model = laphosotdct::findOrFail($id);
    ...
    public function updatedt (Request $request){
        ...
        if(isset($inputs['id'])) {
            $id = $inputs['id'];
            $modelttp = laphosotdct::findOrFail($id);
            $modelttp->madt = $inputs['madt'];
            $modelttp->tendanhhieutd = $inputs['tendanhhieutd'];
            $modelttp->save();

            $model = laphosotdct::where('kihieudhtd', $inputs['kihieudhtd'])
                ->get();
            $result['message'] = '<div class="row" id="dsdt">';
            $result['message'] .= '<div class="col-md-12">';
            $result['message'] .= '<table class="table table-striped table-bordered table-hover" >';
