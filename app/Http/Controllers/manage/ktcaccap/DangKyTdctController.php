...
class DangKyTdctController extends Controller
{
    public function boxungdt(Request $request){
        ...
        $modeltdct = new dangkytdct();
        $inputs['madktdct'] = getdate()[0];
        $modeltdct->create($inputs);
        $m_pl = dmphanloaict::all();
        $modelct = dangkytdct::where('kihieudhtd',$inputs['kihieudhtd'])
            ->get();
        $slcanhan = 0;
        $sltapthe = 0;
    ...
    public function delete(Request $request){
        ...
        if(isset($inputs['id'])){
            ...
            $model = dangkytdct::where('id',$inputs['id'])->first();
            $model->delete();
    ...
    public function chinhsuadt (Request $request){
        ...
        if(isset($inputs['id'])){
            $id = $inputs['id'];
            $model = dangkytdct::findOrFail($id);
    ...
    public function updatedt (Request $request){
        ...
        if(isset($inputs['id'])) {
            $id = $inputs['id'];
            $modelttp = dangkytdct::findOrFail($id);
            $modelttp->madt = $inputs['madt'];
            $modelttp->tendanhhieutd = $inputs['tendanhhieutd'];
            $modelttp->save();

            $model = dangkytdct::where('kihieudhtd', $inputs['kihieudhtd'])
                ->get();
            $result['message'] = '<div class="row" id="dsdt">';
            $result['message'] .= '<div class="col-md-12">';
            $result['message'] .= '<table class="table table-striped table-bordered table-hover" >';
