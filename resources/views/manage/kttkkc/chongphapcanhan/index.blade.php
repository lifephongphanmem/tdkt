...
@section('custom-script')
    ...
    <script>
        ...
        $(function(){
            $('#nam').change(function() {
                var nam = '&nam='+$('#nam').val();
                var url = '/chongphapcanhan?'+nam;
                window.location.href = url;
            });
            $('#loaikt').change(function() {
                var current_path_url = '/chongphapcanhan?';
                var nam = '&nam='+$('#nam').val();
                var loaikt = '&loaikt='+$('#loaikt').val();
                var url = current_path_url + nam + loaikt;
                window.location.href = url;
            });
        });
    </script>
...
@section('content')
    <h3 class="page-title">
        Danh sách<small>&nbsp;khen thưởng kháng chiến chống Mỹ(cá nhân)</small>
    </h3>
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="portlet box">
                <div class="portlet-title">
                    <div class="actions">
                        <a href="{{url('chongphapcanhan/create')}}" class="btn btn-default btn-sm">
                ...
                <div class="portlet-body">
                    <div class="row">
                        ...
                        {{--<div class="col-md-3">
                            <div class="form-group">
                                <label style="font-weight: bold">Loại khen thưởng</label>
                                <select id="loaikt" name="loaikt" class="form-control">
                                    <option value="all">--Tất cả các loại khen thưởng--</option>
                                    @foreach($model as $tt)
                                        <option value="{{$tt->loaikt}}">{{$tt->loaikt}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>--}}
                            ...
                            @foreach($model as $key => $tt)
                                <tr>
                                    ...
                                    <td style="text-align: center">
                                        <a href="{{url('chongphapcanhan/'.$tt->id)}}" class="btn btn-default btn-xs mbs" target="_blank"><i class="fa fa-eye"></i>&nbsp;Xem chi tiết</a>
                                        <a href="{{url('chongphapcanhan/'.$tt->id).'/edit'}}" class="btn btn-default btn-xs mbs"><i class="fa fa-edit"></i>&nbsp;Chỉnh sửa</a>
        ...
        <div class="modal fade" id="delete-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    {!! Form::open(['url'=>'chongphapcanhan/delete','id' => 'frm_delete'])!!}
